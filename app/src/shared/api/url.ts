...
export function resolveMediaUrl(
  ...
  // Some v3.x gateways were deployed without `frontend.publicApiBaseURL` and
  // therefore returned their loopback default (for example
  // `http://127.0.0.1:8000/v1/media/images/...`). The image has already been
  // generated by this authenticated gateway; use the configured endpoint for
  // the same public media path instead of exposing an unusable internal host.
  const gatewayPath = gatewayMediaPath(resolved.pathname, base);
  if (gatewayPath) {
    const configured = new URL(base);
    return `${configured.origin}${gatewayPath}${resolved.search}${resolved.hash}`;
  }
  ...
  if (resolved.protocol === "http:") {
    const baseOrigin = new URL(base).origin;
    if (resolved.origin === baseOrigin) throw new ApiUrlError("Relative media URL must use the gateway media path");
  }
...
function gatewayMediaPath(pathname: string, base: string): string | undefined {
  const normalized = pathname.replace(/\/+$/, "");
  const match = normalized.match(/(\/v1\/media\/(?:images|videos)\/[^/]+)$/i);
